---
icon: lucide/scan-search
---

# Application Preview

This page gives a quick visual tour of the main CVEalert workspaces. Use it to understand where common onboarding and triage tasks happen in the app.

## Preview Map

<div class="grid cards" markdown>

-   :lucide-layout-dashboard:{ .lg .middle } **Daily overview**

    ---

    Start with Dashboard to understand recent alerts, severity trends, and threat signals.

    [:octicons-arrow-right-24: Dashboard](app/dashboard/)

-   :lucide-bell:{ .lg .middle } **Triage alerts**

    ---

    Use Alerts and CVE Detail when deciding what needs investigation or remediation.

    [:octicons-arrow-right-24: Alerts](app/alerts/)

-   :lucide-clock-fading:{ .lg .middle } **Manage software**

    ---

    Use Monitoring, Catalog, and Software CVEs to control what CVEalert tracks.

    [:octicons-arrow-right-24: Software Monitoring](software/monitoring/)

-   :lucide-archive:{ .lg .middle } **Prepare workspace**

    ---

    Use Organization and Integrations to manage team access and alert delivery.

    [:octicons-arrow-right-24: Organization](settings/organization/)

</div>

## Dashboard

Use [Dashboard](app/dashboard/){ data-preview } to check recent alerts, severity trends, affected software, and security news at a glance.

![Dashboard preview](assets/img/preview/01_dashboard.webp){ loading=lazy }
/// caption
Recent alerts and trends
///

---

## Alerts

Use [Alerts](app/alerts/){ data-preview } to triage vulnerabilities detected across monitored software.

![Alerts preview](assets/img/preview/02_alerts.webp){ loading=lazy }
/// caption
Triage detected vulnerabilities
///

---

## CVE Detail

Use [CVE Detail](app/cve/){ data-preview } to investigate severity, affected software, remediation guidance, references, and exploitation context for one CVE.

![CVE Detail preview](assets/img/preview/03_cve.webp){ loading=lazy }
/// caption
Investigate one vulnerability
///

---

## Software Monitoring

Use [Software Monitoring](software/monitoring/){ data-preview } to manage tracked software and tune alert thresholds.

![Software Monitoring preview](assets/img/preview/04_monitoring.webp){ loading=lazy }
/// caption
Manage monitored software
///

---

## Software Catalog

Use [Software Catalog](software/catalog/){ data-preview } to browse known software and add relevant products to monitoring.

![Software Catalog preview](assets/img/preview/05_catalog.webp){ loading=lazy }
/// caption
Add software to monitoring
///

---

## Software CVEs

Use [Software CVEs](software/cves/){ data-preview } to review vulnerabilities linked to software products and narrow results by severity or exploitability signals.

![Software CVEs preview](assets/img/preview/06_cves.webp){ loading=lazy }
/// caption
Search product vulnerabilities
///

---

## Organization

Use [Organization](settings/organization/){ data-preview } to manage the workspace, members, limits, and billing context for your team.

![Organization preview](assets/img/preview/07_organization.webp){ loading=lazy }
/// caption
Manage organization settings
///

---

## Integrations

Use [Integrations](settings/integrations/){ data-preview } to send CVE alerts to Slack or Telegram.

![Integrations preview](assets/img/preview/08_integrations.webp){ loading=lazy }
/// caption
Configure alert integrations
///
